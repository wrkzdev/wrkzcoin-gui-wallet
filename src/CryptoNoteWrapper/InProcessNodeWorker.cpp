...
namespace WalletGui {
	...
	WalletGui::IBlockChainExplorerAdapter* InProcessNodeWorker::getBlockChainExplorerAdapter() {
		...
		try {
			...
			WalletLogger::critical(tr("[Embedded node] Get getBlockChainExplorerAdapter: %1").arg(_error.what()));
		}
		return 0;
	}
	...
	IWalletAdapter* InProcessNodeWorker::getWalletAdapter() {
		...
		try {
			...
			WalletLogger::critical(tr("[Embedded node] Get getWalletAdapter: %1").arg(_error.what()));
		}
		return 0;
	}
